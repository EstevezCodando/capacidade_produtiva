  ...
  return (
    <div className={styles.dropdown} role="dialog" aria-label="Alertas">
      ...
      <div className={styles.dropdownBody}>
        {isLoading && <p className={styles.dropdownEmpty}>Carregando…</p>}
        ...
        {itens.slice(0, 8).map((item) => (
          <div key={item.ut_id} className={styles.alertItem}>
            <span className={styles.alertIcon}>
              {sevIcon[item.ocorrencia] ?? "!"}
            </span>
            <div className={styles.alertContent}>
              <span className={styles.alertTitle}>
                UT {item.ut_id} — {item.ocorrencia.replace(/_/g, " ")}
              </span>
              <span className={styles.alertMeta}>
                {item.projeto_nome ?? "—"} · {item.subfase_nome ?? "—"} ·{" "}
                {item.nome_executor ?? "—"}
              </span>
            </div>
          </div>
        ...
        {itens.length > 8 && (
          <p className={styles.dropdownMore}>
            +{itens.length - 8} ocorrências — ver em Atividades
          </p>
        )}
      </div>
...
function PerfilDropdown({ onClose }: { onClose: () => void }) {
  ...
  const loginEm = sessao?.login_em
    ? formatDistanceToNow(new Date(sessao.login_em), {
        addSuffix: true,
        locale: ptBR,
      })
    : "—";
    ...
    <div className={styles.dropdown} role="dialog" aria-label="Perfil">
      ...
      <div className={styles.perfilHeader}>
        ...
        <div>
          ...
          <span
            className={`${styles.perfilBadge} ${ehAdmin ? styles.perfilBadgeAdmin : styles.perfilBadgeOp}`}
          >
      ...
      <div className={styles.perfilAcoes}>
        ...
        <button
          className={`${styles.perfilAcao} ${styles.perfilAcaoLogout}`}
...
export default function TopNav() {
  ...
  useEffect(() => {
    function handler(e: MouseEvent) {
      if (
        alertasRef.current &&
        !alertasRef.current.contains(e.target as Node)
      ) {
    ...
    <header className={styles.nav}>
      ...
      <div className={styles.left}>
        <div className={styles.logo}>
          ...
          <span className={styles.logoName}>Capacidade Produtiva</span>
        </div>
      ...
      <div className={styles.right}>
        ...
        {ehAdmin && (
          <div className={styles.dropdownWrap} ref={alertasRef}>
            ...
            {alertasAberto && (
              <AlertasDropdown onClose={() => setAlertasAberto(false)} />
            )}
          </div>
        )}
        ...
        <div className={styles.dropdownWrap} ref={perfilRef}>
          ...
          {perfilAberto && (
            <PerfilDropdown onClose={() => setPerfilAberto(false)} />
          )}
        </div>
      </div>
    </header>
