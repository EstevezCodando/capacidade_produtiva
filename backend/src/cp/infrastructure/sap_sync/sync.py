...
def _upsert_quando_mudou(
    ...
    set_str = ", ".join(f"{c}=EXCLUDED.{c}" for c in cols if c != pk)
    where = " OR ".join(
        f"{_SCHEMA}.{tabela}.{c} IS DISTINCT FROM EXCLUDED.{c}"
        for c in cols
        if c != pk
    )
    sql = text(
...
def _sync_unidade_trabalho(s: Connection, c: Connection) -> ResultadoTabela:
    ...
    return _sync(
        s, c, "macrocontrole_unidade_trabalho",
        ...
        "id", ["id", "nome", "epsg", "dado_producao_id", "subfase_id",
               "lote_id", "bloco_id", "disponivel", "dificuldade",
...
def _sync_atividade(s: Connection, c: Connection) -> ResultadoTabela:
    ...
    cols = ["id", "etapa_id", "unidade_trabalho_id", "usuario_id",
            "tipo_situacao_id", "data_inicio", "data_fim", "observacao"]
...
def _sync_perfil_producao(s: Connection, c: Connection) -> ResultadoTabela:
    return _sync(s, c, "macrocontrole_perfil_producao",
...
def sincronizar_sap_para_snapshot(
    ...
    with engine_cp.begin() as conn_cp, engine_sap.connect() as conn_sap:
        for fn in _PIPELINE:
            resultados.append(fn(conn_sap, conn_cp))  # type: ignore[operator]
    return resultados
