...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("disparado_por", sa.Text(), nullable=False),
    ...
    op.create_index(
        "ix_log_ingestao_execucao_finalizado_em",
        "ingestao_execucao",
        ["finalizado_em"],
        schema="log",
    )
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_ingestao_execucao_em_andamento
        ON log.ingestao_execucao ((status))
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS log.uq_ingestao_execucao_em_andamento")
    op.drop_index("ix_log_ingestao_execucao_status", table_name="ingestao_execucao", schema="log")
    op.drop_index("ix_log_ingestao_execucao_finalizado_em", table_name="ingestao_execucao", schema="log")
